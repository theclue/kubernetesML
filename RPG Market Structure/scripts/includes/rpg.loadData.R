...
# rpg.loadData
# ------------------------
# Get data from text file from my s3 repo, unpack
# and load into a data frame
###########

rpg.loadData <- function(file, repo = "http://s3.amazonaws.com/extras.gabrielebaldassarre.com/datascience/rpg", output.dir = "./data", overwrite = FALSE, ...){
  ...
  if (
    ...
    (read.table(textConnection(getURL(paste(repo, paste(file, "md5", sep="."), sep="/"))), sep=",", header=FALSE)[1] != (md5sum(paste(output.dir, file, sep="/"))))
    ) {
    download.file(paste(repo, file, sep="/"), paste(output.dir, file, sep="/"))
  }
...
rpg.saveData <- function(data.frame, file = NULL, output.dir = "./data", overwrite = TRUE, repo = "http://s3.amazonaws.com/extras.gabrielebaldassarre.com/datascience/rpg", ...){
